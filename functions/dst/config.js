...
import { MoneymanConfigSchema, BooleanEnvVarSchema, } from "./config.schema.js";
...
function createConfig() {
    ...
    if (MONEYMAN_CONFIG) {
        ...
        try {
            ...
            logger("Failed to parse MONEYMAN_CONFIG. Unable to continue with invalid configuration", error);
    ...
    if (MONEYMAN_CONFIG_PATH) {
        ...
        try {
            ...
            logger("Failed to parse config file from MONEYMAN_CONFIG_PATH. Unable to continue with invalid configuration", error);
            void sendConfigError(error);
        }
    }
    logToPublicLog("No configuration found. Please provide MONEYMAN_CONFIG or MONEYMAN_CONFIG_PATH environment variable.");
...
function getAccounts() {
    ...
    return !accountsToScrape || accountsToScrape.length === 0
        ? allAccounts
        : allAccounts.filter((account) => accountsToScrape.includes(account.companyId));
}
...
async function sendConfigError(error) {
    ...
    if (MONEYMAN_CONFIG) {
        ...
        try {
            ...
            const configFileContent = readFileSync(MONEYMAN_CONFIG_PATH, "utf-8");
            const config = parseJsoncConfig(configFileContent);
            if (config.options?.notifications?.telegram) {
                const { apiKey, chatId } = config.options.notifications.telegram;
                await new Telegraf(apiKey).telegram.sendMessage(chatId, message);
            }
