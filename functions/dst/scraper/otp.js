 ...
 */
function createOtpCodeRetriever(account) {
    return async () => {
        ...
        logger(`Requesting OTP code for ${account.companyId} account (phone: ${phoneNumber})`);
 ...
 */
export function shouldCreateOtpRetriever(account) {
    return (account.companyId === "oneZero" &&
 ...
 */
export function prepareAccountCredentials(account) {
