...
export async function scrapeAccounts({ accounts, startDate, futureMonthsToScrape, parallelScrapers, additionalTransactionInformation, includeRawTransaction, }, scrapeStatusChanged, onError) {
    const start = performance.now();
    logger(`scraping %d accounts`, accounts.length);
    logger(`start date %s`, startDate.toISOString());
    let futureMonths = undefined;
    ...
    const results = await parallelLimit(accounts.map((account, i) => async () => {
        const { companyId } = account;
        return loggerContextStore.run({ prefix: `[#${i} ${companyId}]` }, async () => scrapeAccount(account, {
...
async function scrapeAccount(account, scraperOptions, setStatusMessage) {
    logger(`scraping started`);
    const scraperStart = performance.now();
    const result = await getAccountTransactions(account, scraperOptions, (cid, step) => setStatusMessage(`[${cid}] ${step}`));
