...
const MM_PREFIX = "mm_";
export class MoneymanDashStorage {
    config;
    endpoint;
    token;
    lastRunId;
    ...
    parseToken() {
        ...
        if (!tokenString.startsWith(MM_PREFIX)) {
            throw new Error("Invalid token format: expected mm_<base64url encoded token>");
        }
        ...
        if (!decoded.u || !decoded.k) {
            throw new Error("Invalid mm_ token: missing required fields 'u' (URL) or 'k' (secret)");
        }
    ...
    async saveTransactions(txns, onProgress, context) {
        ...
        if (!response.ok) {
            ...
            logger(`Failed to post transactions: ${response.status} ${response.statusText}${detail}`);
            throw new Error(`Failed to post transactions: ${response.status} ${response.statusText}${detail}`);
        }
        const result = (await response.json());
        if (typeof result.transactionsAdded !== "number") {
            throw new Error(`Unexpected ingestion response shape: ${JSON.stringify(result)}`);
        }
        logger(`Ingestion complete: ${result.transactionsAdded} transactions added`);
    ...
    async sendLogs(logs) {
        ...
        try {
            ...
            if (!response.ok) {
                const body = await response.text().catch(() => "");
                const detail = body ? `: ${body}` : "";
                logger(`Failed to upload logs: ${response.status} ${response.statusText}${detail}`);
                return;
            }
    ...
    buildIngestionPayload(txns, context) {
...
//# sourceMappingURL=moneyman.js.map