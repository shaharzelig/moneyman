...
describe("SqlStorage", () => {
    ...
    it("creates schema and inserts completed transactions", async () => {
        ...
        const rows = db.public.many(`SELECT "unique_id", "description", "raw"->>'hash' AS raw_hash
       FROM "moneyman"."transactions"`);
        ...
        const rawRows = db.public.many(`SELECT "raw"->>'uniqueId' AS unique_id, "raw"->>'hash' AS raw_hash, "raw"->>'description' AS raw_description
    ...
    it("upserts existing transactions and appends to raw log", async () => {
        ...
        const existing = db.public.one(`SELECT "unique_id" FROM "moneyman"."transactions" WHERE "unique_id" = '${tx.uniqueId}'`);
        ...
        const stored = db.public.one(`SELECT "description", "memo" FROM "moneyman"."transactions" WHERE "unique_id" = '${updated.uniqueId}'`);
        expect(stored).toMatchObject({
            description: "Updated description",
            memo: "Updated memo",
        });
        const rawCount = db.public.one(`SELECT COUNT(*)::int AS count FROM "moneyman"."transactions_raw" WHERE "raw"->>'uniqueId' = '${updated.uniqueId}'`);
    ...
    it("skips pending transactions from main table but records them in raw log", async () => {
        ...
        const storedRows = db.public.many(`SELECT * FROM "moneyman"."transactions" WHERE "unique_id" = '${pending.uniqueId}'`);
        expect(storedRows).toHaveLength(0);
        const rawStored = db.public.one(`SELECT "raw"->>'status' AS status FROM "moneyman"."transactions_raw" WHERE "raw"->>'uniqueId' = '${pending.uniqueId}'`);
    ...
    it("counts duplicate unique IDs within a single batch as updates", async () => {
        ...
        const stats = await storage.saveTransactions([first, duplicate], onProgress);
        expect(stats.added).toBe(1);
        expect(stats.existing).toBe(1);
        expect(stats.pending).toBe(0);
        const stored = db.public.one(`SELECT "description", "memo", "raw"->>'hash' AS raw_hash FROM "moneyman"."transactions" WHERE "unique_id" = 'duplicate-1'`);
        expect(stored).toMatchObject({
            description: "Updated description",
            memo: "Updated memo",
            raw_hash: "hash-updated",
        });
        const rawRows = db.public.many(`SELECT "raw"->>'hash' AS hash FROM "moneyman"."transactions_raw" WHERE "raw"->>'uniqueId' = 'duplicate-1' ORDER BY "id"`);
    ...
    it("mixes new and existing transactions in one batch", async () => {
        ...
        const stats = await storage.saveTransactions([updatedExisting, newlyInserted], onProgress);
        expect(stats.added).toBe(1);
        expect(stats.existing).toBe(1);
        expect(stats.pending).toBe(0);
        const rows = db.public.many(`SELECT "unique_id", "description", "raw"->>'hash' AS raw_hash FROM "moneyman"."transactions" WHERE "unique_id" IN ('existing-1', 'existing-2') ORDER BY "unique_id"`);
        expect(rows).toEqual([
            {
        ...
        const rawCounts = db.public.many(`SELECT "raw"->>'uniqueId' AS unique_id, COUNT(*)::int AS count FROM "moneyman"."transactions_raw" WHERE "raw"->>'uniqueId' IN ('existing-1', 'existing-2') GROUP BY "raw"->>'uniqueId' ORDER BY "raw"->>'uniqueId'`);
