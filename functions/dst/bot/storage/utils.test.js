...
describe("transactionHash", () => {
    ...
    it("should return different hashes for different company ids", () => {
        const accountNumber = "123";
        const hash1 = transactionHash(transaction1, CompanyTypes.leumi, accountNumber);
        const hash2 = transactionHash(transaction1, CompanyTypes.isracard, accountNumber);
        expect(hash1).not.toBe(hash2);
    });
    ...
    it("Issue #132: hash has no nullish string", () => {
        ...
        for (const transaction of [transactionWithNull, transactionWithUndefined]) {
            const hash = transactionHash(transaction, CompanyTypes.leumi, "123");
    ...
    describe("error handling for malformed transactions", () => {
        it("should throw error when transaction has null date", () => {
            ...
            expect(() => {
                transactionHash(transactionWithNullDate, CompanyTypes.hapoalim, "123");
            }).toThrow();
        });
        it("should throw error when transaction has undefined date", () => {
            ...
            expect(() => {
                transactionHash(transactionWithUndefinedDate, CompanyTypes.hapoalim, "123");
            }).toThrow();
        });
        it("should throw error when transaction has invalid date string", () => {
            ...
            expect(() => {
                transactionHash(transactionWithInvalidDate, CompanyTypes.hapoalim, "123");
            }).toThrow();
        });
    });
});
describe("transactionUniqueId", () => {
    ...
    it("should return different unique ids for different company ids", () => {
        const accountNumber = "123";
        const id1 = transactionUniqueId(transaction1, CompanyTypes.leumi, accountNumber);
        const id2 = transactionUniqueId(transaction1, CompanyTypes.isracard, accountNumber);
        expect(id1).not.toBe(id2);
    });
    ...
    it('should return different unique ids for transactions with different "identifier" values', () => {
        ...
        expect(transactionHash(tx1, companyId, accountNumber)).toBe(transactionHash(tx2, companyId, accountNumber));
        // The unique id will be different because it includes the identifier
        expect(transactionUniqueId(tx1, companyId, accountNumber)).not.toBe(transactionUniqueId(tx2, companyId, accountNumber));
    });
    it.each([undefined, null, ""])('should fallback to "description" + "memo" if "identifier" is %o', (identifier) => {
        const tx = {
            ...transaction1,
            identifier,
        };
        const id = transactionUniqueId(tx, CompanyTypes.hapoalim, "123");
