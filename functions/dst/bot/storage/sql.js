...
export class SqlStorage {
    ...
    async saveTransactions(txns, onProgress) {
        ...
        const stats = createSaveStats("Postgres", `${schema}.${TRANSACTIONS_TABLE_NAME}`, txns);
        const pool = new Pool({
        ...
        try {
            ...
            const nonPending = txns.filter((tx) => tx.status !== TransactionStatuses.Pending);
            await this.withTransaction(client, async (transactionClient) => {
                if (nonPending.length > 0) {
                    await onProgress(`Upserting ${nonPending.length} transactions`);
                    const { inserted, updated } = await this.upsertTransactions(transactionClient, nonPending, schema);
    ...
    async ensureInitialized(client, schema) {
        logger(`Ensuring schema ${schema} exists`);
        await client.query(pgFormat("CREATE SCHEMA IF NOT EXISTS %I", schema));
        await client.query(`
      CREATE TABLE IF NOT EXISTS ${pgFormat("%I.%I", schema, TRANSACTIONS_TABLE_NAME)} (
    ...
    `);
        await client.query(pgFormat(`CREATE INDEX IF NOT EXISTS %I ON %s ("company_id", "account", "activity_date")`, `${TRANSACTIONS_TABLE_NAME}_company_account_date_idx`, pgFormat("%I.%I", schema, TRANSACTIONS_TABLE_NAME)));
    }
    async upsertTransactions(client, txns, schema) {
        if (txns.length === 0) {
            return { inserted: 0, updated: 0 };
        }
        const uniqueIds = Array.from(new Set(txns.map((tx) => tx.uniqueId)));
        const existingRows = uniqueIds.length
            ? await client.query(`SELECT "unique_id" FROM ${pgFormat("%I.%I", schema, TRANSACTIONS_TABLE_NAME)} WHERE "unique_id" IN (${uniqueIds
                .map((_, index) => `$${index + 1}`)
                .join(", ")})`, uniqueIds)
            : { rows: [] };
        const existingIds = new Set(existingRows.rows.map((row) => row.unique_id));
        const insert = buildInsertStatement(schema, TRANSACTIONS_TABLE_NAME, [...TRANSACTION_COLUMNS], txns.map((tx) => toRow(tx)));
        await client.query({
            text: `${insert.text}
    ...
    async insertRawTransactions(client, txns, schema) {
        if (txns.length === 0) {
            return;
        }
        await client.query(buildInsertStatement(schema, RAW_TABLE_NAME, ["raw"], txns.map((tx) => toRow(tx))));
    }
}
function toRow(tx) {
    ...
    return {
        ...
        installments: tx.installments === undefined || tx.installments === null
            ? null
