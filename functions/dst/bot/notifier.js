...
logToPublicLog(bot
    ...
    : "No Telegram bot info, status and errors will not be sent", logger);
...
export async function send(message, parseMode) {
...
export async function sendPhoto(photoPath, caption) {
    ...
    return await bot.telegram.sendPhoto(telegramConfig.chatId, { source: photoPath }, { caption, has_spoiler: true });
}
export async function sendPhotos(photos) {
    logger(`Sending photos`, { photos });
    if (photos.length === 0 || !bot || !telegramConfig?.chatId) {
        return;
    }
    return await bot.telegram.sendMediaGroup(telegramConfig.chatId, photos.map(({ photoPath, caption }) => ({
        type: "photo",
        caption,
        media: { source: photoPath },
    })));
}
export async function sendJSON(json, filename) {
...
export async function sendTextFile(filePath, caption) {
    logger(`Sending file`, { filePath, caption });
    if (!bot || !telegramConfig?.chatId) {
        return;
    }
    return await bot.telegram.sendDocument(telegramConfig.chatId, { source: filePath, filename: `${filePath}.txt` }, caption ? { caption } : undefined);
}
export async function editMessage(message, newText, parseMode) {
    ...
    try {
        ...
        await bot.telegram.editMessageText(telegramConfig.chatId, message, undefined, newText, {
            parse_mode: parseMode,
        });
    }
    catch (e) {
...
function canIgnoreTelegramError(e) {
    return (e instanceof TelegramError &&
        e.response.description.startsWith("Bad Request: message is not modified"));
}
export function sendError(message, caller = "") {
    return send(`${caller}\n❌ ${String(message instanceof Error
        ? `${message.message}\n${message.stack}`
        : message)}`.trim());
}
/**
 ...
 */
export async function requestOtpCode(companyId, phoneNumber) {
    if (!bot || !telegramConfig?.chatId || !telegramConfig.enableOtp) {
        throw new Error("Telegram OTP is not enabled or configured");
    }
    const requestMessage = await send(`🔐 2FA Authentication Required\n\n` +
        `Account: ${companyId}\n` +
    ...
    const timeoutPromise = waitForAbortSignal(AbortSignal.timeout(timeoutSeconds * 1000)).catch(() => {
        throw new Error(`OTP timeout: No response received within ${timeoutSeconds} seconds`);
    });
