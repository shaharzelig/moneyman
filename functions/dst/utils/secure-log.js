...
export function enableDebugLoggingIfNeeded() {
    if (config.options.logging.debugFilter &&
        !unsafeStdout &&
        !process.env.DEBUG) {
...
export async function sendAndDeleteLogFile() {
    ...
    const hasAnyLogDest = (telegram?.chatId && telegram?.sendLogFileToTelegram) ||
        logStorages.length > 0;
    if (!hasAnyLogDest) {
        logToPublicLog("⚠️  WARNING: Output is redirected to a log file but no log destinations are configured. Errors and logs will not be visible", logger);
    }
    try {
        ...
        if (logStorages.length > 0) {
            ...
            const results = await Promise.allSettled(logStorages.map(async (storage) => {
...
//# sourceMappingURL=secure-log.js.map