...
// In headed mode or when MONEYMAN_STDIN_OTP=true, attach a stdin OTP retriever
if (process.env.PUPPETEER_HEADLESS === "false" || process.env.MONEYMAN_STDIN_OTP === "true") {
    const cibusAccount = scraperConfig.accounts.find((a) => a.companyId === "cibus");
    if (cibusAccount) {
        cibusAccount.otpCodeRetriever = () => new Promise((resolve) => {
            const rl = createInterface({
                input: process.stdin,
                output: process.stderr,
            });
            rl.question("\n>>> Enter Cibus OTP code from SMS: ", (code) => {
                rl.close();
                resolve(code.trim());
            });
        });
        logger("Stdin OTP retriever attached for cibus");
    }
}
