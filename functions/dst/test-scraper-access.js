...
import { createBrowser, createSecureBrowserContext, } from "./scraper/browser.js";
...
debug.enable("moneyman:browser,moneyman:test-scraper-access,moneyman:cloudflare-solver,moneyman:domain-rules,israeli-bank-scrapers:*");
        ...
        for (const { company, url, titles, expectedText } of [
            ...
            test(`should access ${company} at ${url}`, async () => {
                ...
                if (!titles.some((title) => initialPageTitle.includes(title))) {
                    logger(`Page title does not yet match, waiting for navigation. was ${initialPageTitle}`);
                ...
                assert.ok(titles.some((title) => pageTitle.includes(title)), `Expected page title to include one of: ${titles.join(", ")}`);
                const textFound = await page
                    .mainFrame()
                    .evaluate((expectedText) => document.body.innerText.includes(expectedText), expectedText);
...
//# sourceMappingURL=test-scraper-access.js.map