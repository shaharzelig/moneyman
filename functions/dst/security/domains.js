...
const domainsFromNode = new Map();
const pagesByCompany = new Map();
const blockedByCompany = new Map();
const allowedByCompany = new Map();
const resourceTypesByCompany = new Map();
export function monitorNodeConnections() {
    if (scrapingConfig.domainTracking) {
        const interceptor = new ClientRequestInterceptor();
        interceptor.apply();
        interceptor.on("request", runInLoggerContext(({ request }) => {
...
export async function initDomainTracking(browserContext, companyId) {
    if (scrapingConfig.domainTracking) {
        ...
        const rules = new DomainRuleManager(companyId, securityConfig.firewallSettings ?? [], securityConfig.blockByDefault);
        browserContext.on("targetcreated", runInLoggerContext(async (target) => {
            switch (target.type()) {
                ...
                case TargetType.BACKGROUND_PAGE: {
                    ...
                    page.on("request", runInLoggerContext(async (request) => {
                        ...
                        addToKeyedSet(resourceTypesByCompany.get(reqKey), companyId, resourceType);
                        if (canIntercept) {
...
export async function getUsedDomains() {
    ...
    const domainsRecord = Object.fromEntries(Array.from(allCompanies).map((company) => {
        function getArray(set) {
            return Array.from(set?.get(company) ?? []);
        }
        function withResourceType(key) {
    ...
    const nodeDomains = Object.fromEntries(Array.from(domainsFromNode.entries()).map(([context, domains]) => [
        context,
        Array.from(domains),
    ]));
