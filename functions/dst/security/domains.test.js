...
describe("domains", () => {
    ...
    describe("initDomainTracking", () => {
        ...
        it("should set up event listeners when domain tracking is enabled", async () => {
            ...
            const { initDomainTracking } = await import("./domains.js");
            await initDomainTracking(browserContext, CompanyTypes.max);
            expect(browserContext.on).toHaveBeenCalledWith("targetcreated", expect.any(Function));
        });
        it("should set up page request listener for relevant target types", async () => {
            ...
            expect(page.on).toHaveBeenNthCalledWith(1, "framenavigated", expect.any(Function));
            expect(page.on).toHaveBeenNthCalledWith(2, "request", expect.any(Function));
            const [[, framenavigated], [, request]] = page.on.mock.calls;
            const framenavigatedCallback = framenavigated;
            framenavigatedCallback(mock({ url: () => "https://bar.com/hello" }));
...
function mockHttpRequest(url, { method = "GET", resourceType = "document" } = {}) {
    const req = mock();
    ...
    req.resourceType.mockReturnValue(resourceType);
    return req;
}
