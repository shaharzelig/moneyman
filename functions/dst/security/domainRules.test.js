...
describe("domainRules", () => {
    describe("DomainRuleManager", () => {
        it("should load rules from the provided string", () => {
            ...
            const result = ruleManager.getRule("https://api.bankhapoalim.co.il/login");
    ...
    describe("DomainRuleManager blockByDefault behavior", () => {
        ...
        it("should default to ALLOW when blockByDefault is explicitly false", () => {
            const ruleManager = new DomainRuleManager(CompanyTypes.hapoalim, [], false);
            // Test with no rules defined - should default to ALLOW
            expect(ruleManager.getRule("https://example.com")).toBe("ALLOW");
            expect(ruleManager.getRule("https://unknown-domain.com")).toBe("ALLOW");
        });
        it("should default to BLOCK when blockByDefault is true", () => {
            const ruleManager = new DomainRuleManager(CompanyTypes.hapoalim, [], true);
        ...
        it("should respect explicit rules regardless of blockByDefault setting", () => {
            ...
            const allowByDefaultManager = new DomainRuleManager(CompanyTypes.hapoalim, rules, false);
            expect(allowByDefaultManager.getRule("https://api.bankhapoalim.co.il")).toBe("ALLOW");
            expect(allowByDefaultManager.getRule("https://malicious.com")).toBe("BLOCK");
            expect(allowByDefaultManager.getRule("https://unknown.com")).toBe("ALLOW"); // Default
            // Test with blockByDefault = true
            const blockByDefaultManager = new DomainRuleManager(CompanyTypes.hapoalim, rules, true);
            expect(blockByDefaultManager.getRule("https://api.bankhapoalim.co.il")).toBe("ALLOW");
            expect(blockByDefaultManager.getRule("https://malicious.com")).toBe("BLOCK");
            expect(blockByDefaultManager.getRule("https://unknown.com")).toBe("BLOCK"); // Default
        });
    });
    // GLOBAL rules not supported in this change
});
//# sourceMappingURL=domainRules.test.js.map